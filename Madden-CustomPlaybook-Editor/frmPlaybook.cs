...
namespace MaddenCustomPlaybookEditor
{
    public partial class frmPlaybook : Form
    {
        ...
        private List<PBFM> PBFM;
        private List<SETL> SETL;
        private List<STID> STID;
        private List<PGPL> PGPL;
        private List<PBPL> PBPL;
        private List<PBFI> PBFI;
        public List<PLPD> PLPD;
        public List<PLRD> PLRD;
        public List<PLYS> PLYS;
        private List<SETP> SETP;
        private List<SPKF> SPKF;
        private List<SPKG> SPKG;
        ...
        private List<PBAI> PBAI;
        private List<PBAU> PBAU;
        private List<PLCM> PLCM;
        private List<PPCT> PPCT;
        private List<SDEF> SDEF;
        ...
        public frmPlaybook()
        {
            ...
            cdlPSLO.CustomColors = new int[] {
                                        ColorTranslator.ToOle(PSALColor.BaseRoute.Color),
                                        ColorTranslator.ToOle(PSALColor.PrimaryRoute.Color),
                                        ColorTranslator.ToOle(PSALColor.DelayRoute.Color),
                                        ColorTranslator.ToOle(PSALColor.MotionRoute.Color),
                                        ColorTranslator.ToOle(PSALColor.Run.Color),
                                        ColorTranslator.ToOle(PSALColor.QBScramble.Color),
                                        ColorTranslator.ToOle(PSALColor.QBHandoff.Color),
                                        ColorTranslator.ToOle(PSALColor.Kickoff.Color)
                                      };
            cdlPSLD.CustomColors = new int[] {
                                        ColorTranslator.ToOle(PSALColor.CloudFlat.Color),
                                        ColorTranslator.ToOle(PSALColor.HardFlat.Color),
                                        ColorTranslator.ToOle(PSALColor.SoftSquat.Color),
                                        ColorTranslator.ToOle(PSALColor.MidRead.Color),
                                        ColorTranslator.ToOle(PSALColor.ThreeReceiverHook.Color),
                                        ColorTranslator.ToOle(PSALColor.VertHook.Color),
                                        ColorTranslator.ToOle(PSALColor.CurlFlat.Color),
                                        ColorTranslator.ToOle(PSALColor.SeamFlat.Color),
                                        ColorTranslator.ToOle(PSALColor.QuarterFlat.Color),
                                        ColorTranslator.ToOle(PSALColor.DeepZone.Color),
                                        ColorTranslator.ToOle(PSALColor.QBSpy.Color),
                                        ColorTranslator.ToOle(PSALColor.RushQB.Color)
                                      };
        ...
        private void mnuOptions_Field_Default_Click(object sender, EventArgs e)
        {
            ...
            mnuOptions_Field_Custom.Checked = false;
        ...
        private void mnuOptions_Field_Custom_Click(object sender, EventArgs e)
        {
            colorDialog1.ShowDialog();
            SolidBrush SBcolor = new SolidBrush(colorDialog1.Color);
            Bitmap bmp = new Bitmap(533, 600);
            using (Graphics graph = Graphics.FromImage(bmp))
            {
                Rectangle ImageSize = new Rectangle(0, 0, 533, 600);
                graph.FillRectangle(SBcolor, ImageSize);
            }
            picPlay.Image = bmp;
            blankField = picPlay.Image;
            mnuOptions_Field_Default.Checked = false;
            ...
            mnuOptions_Field_Dark.Checked = false;
            mnuOptions_Field_Offense.Checked = false;
            mnuOptions_Field_Custom.Checked = true;
            mnuOptions_Field_Transparent.Checked = false;
            if (!(Play.PBPL == null)) UpdatePlay(Play);
        }
        ...
        private void cbxFormations_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                cmsEditFormation.Show(Cursor.Position.X, Cursor.Position.Y);
            }
        }
        ...
        private void EditFormation_Click(object sender, EventArgs e)
        {
            ...
            frmEditFormation.DefaultSETG = (from row in SETG select new SETG()
            {
                rec = row.rec,
                SETP = row.SETP,
                SGF_ = row.SGF_,
        ...
        private void cmsEditPSAL_Color_Click(object sender, EventArgs e)
        {
            ...
            Play.posos[dgvPLYS.CurrentRow.Index].RouteColor = new PSALColor{ Color = cdlPSLO.Color, Type = Play.posos[dgvPLYS.CurrentRow.Index].RouteColor.Type };
        ...
        private void selectAlignment(object sender, EventArgs e)
        {
            try
            { 
                if (cbxPlays.Items.Count == 0) UpdatePlays();
                Play = GetPlay();
                UpdatePlay(Play); 
            }
            catch{}
        ...
        private List<TableName> GetTables()
        {
            //Number of tables
            ...
            for (int i = 0; i < TableCount; i++)
            {
                ...
                if (TDB.TDBTableGetProperties(OpenIndex, i, ref TableProps))
                {
                    ...
                    tablenames.Add(new TableName { rec = i, name = TableProps.Name });
                }
            }
            return tablenames;
        }
        ...
        private void UpdateDB()
        {
            ...
            TDB.TDBTableGetProperties(OpenIndex, TableNames.Find(item => item.name == "SETG").rec, ref TableProps);

            if (SETG.Count() > TableProps.RecordCount)
            {
                for (int i = 0; i < SETG.Count() - TableProps.RecordCount; i++)
                {
                    TDB.TDBTableRecordAdd(OpenIndex, StrReverse("SETG"), true);
                }
            }
            else if (SETG.Count() < TableProps.RecordCount)
            {
                for (int i = 0; i < TableProps.RecordCount - SETG.Count(); i++)
                {
                    TDB.TDBTableRecordRemove(OpenIndex, StrReverse("SETG"), i);
                }
            }
        ...
        private List<PBFM> GetPBFM()
        {
            List<PBFM> _PBFM = new List<PBFM>();

            TdbTableProperties TableProps = new TdbTableProperties();
            TableProps.Name = new string((char)0, 5);

            // Get Tableprops based on the selected index
            if (!TDB.TDBTableGetProperties(OpenIndex, TableNames.Find(item => item.name == "PBFM").rec, ref TableProps))
                return null;
            ...
            for (int i = 0; i < TableProps.RecordCount; i++)
            {
                ...
                TDB.TDBFieldGetProperties(OpenIndex, StrReverse("PBFM"), 7, ref FieldProps);

                string _name = new string((char)0, (FieldProps.Size / 8) + 1);

                TDB.TDBFieldGetValueAsString(OpenIndex, StrReverse("PBFM"), StrReverse("name"), i, ref _name);
                _name = _name.Replace(",", "");

                _PBFM.Add(new PBFM
                {
                    rec = i,
                    FAU1 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBFM"), StrReverse("FAU1"), i),
                    FAU2 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBFM"), StrReverse("FAU2"), i),
                    FAU3 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBFM"), StrReverse("FAU3"), i),
                    FAU4 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBFM"), StrReverse("FAU4"), i),
                    pbfm = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBFM"), StrReverse("PBFM"), i),
                    FTYP = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBFM"), StrReverse("FTYP"), i),
                    ord_ = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBFM"), StrReverse("ord_"), i),
                    name = _name
                });
            }
        ...
        public List<STID> GetSTID()
        {
            ...
            // Get Tableprops based on the selected index
            if (!TDB.TDBTableGetProperties(OpenIndex, TableNames.Find(item => item.name == "STID").rec, ref TableProps))
                return null;
            ...
            for (int i = 0; i < TableProps.RecordCount; i++)
            {
                ...
                {
                    rec = i,
                    BOKL = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("STID"), StrReverse("BOKL"), i),
                    SETL = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("STID"), StrReverse("SETL"), i),
                    PBFM = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("STID"), StrReverse("PBFM"), i),
                    PBST = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("STID"), StrReverse("PBST"), i),
                    SPF_ = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("STID"), StrReverse("SPF_"), i)
                });
            }
            return _STID;
        }

        public List<PGPL> GetPGPL()
        {
            ...
            // Get Tableprops based on the selected index
            if (!TDB.TDBTableGetProperties(OpenIndex, TableNames.Find(item => item.name == "PGPL").rec, ref TableProps))
                return null;
            ...
            for (int i = 0; i < TableProps.RecordCount; i++)
            {
                ...
                {
                    rec = i,
                    BOKL = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PGPL"), StrReverse("BOKL"), i),
                    SETL = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PGPL"), StrReverse("SETL"), i),
                    PLYL = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PGPL"), StrReverse("PLYL"), i),
                    PBST = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PGPL"), StrReverse("PBST"), i),
                    PLYT = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PGPL"), StrReverse("PLYT"), i),
                    ord_ = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PGPL"), StrReverse("ord_"), i),
                    Flag = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PGPL"), StrReverse("Flag"), i)
                });
            }
            return _PGPL;
        }
        ...
        private List<PBFI> GetPBFI()
        {
            List<PBFI> _PBFI = new List<PBFI>();

            TdbTableProperties TableProps = new TdbTableProperties();
            TableProps.Name = new string((char)0, 5);

            // Get Tableprops based on the selected index
            if (!TDB.TDBTableGetProperties(OpenIndex, TableNames.Find(item => item.name == "PBFI").rec, ref TableProps))
                return null;
            ...
            for (int i = 0; i < TableProps.RecordCount; i++)
            {
                ...
                TDB.TDBFieldGetProperties(OpenIndex, StrReverse("PBFI"), 1, ref FieldProps);

                string _name = new string((char)0, (FieldProps.Size / 8) + 1);

                TDB.TDBFieldGetValueAsString(OpenIndex, StrReverse("PBFI"), StrReverse("name"), i, ref _name);
                _name = _name.Replace(",", "");

                _PBFI.Add(new PBFI
                {
                    rec = i,
                    BOKL = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBFI"), StrReverse("BOKL"), i),
                    name = _name
                });
            }
            return _PBFI;
        ...
        public PLAY GetPlay()
        {
            ...
            #region Get PGPL for current play

            _PLAY.PGPL = new List<PGPL>();

            for (int i = 0; i < PGPL.Count; i++)
            {
                if (PGPL[i].PLYL == ((PBPL)cbxPlays.SelectedItem).PLYL && PGPL[i].SETL == ((SETL)cbxSubFormations.SelectedItem).setl)
                {
                    _PLAY.PGPL.Add(new PGPL
                    {
                        rec = PGPL[i].rec,
                        BOKL = PGPL[i].BOKL,
                        SETL = PGPL[i].SETL,
            ...
            #region Get SPKF for current play

            if (SPKF != null)
            {
                _PLAY.SPKF = SPKF.Where(package => package.SETL == ((PBPL)cbxPlays.SelectedItem).SETL).Cast<SPKF>().ToList();
                _PLAY.SPKG = new List<SPKG>();
                foreach (SPKF package in _PLAY.SPKF)
                {
                    _PLAY.SPKG.AddRange(SPKG.Where(sub => sub.SPF_ == package.SPF_).Cast<SPKG>().ToList());
                }

                Console.WriteLine(_PLAY.SPKF);
                Console.WriteLine(_PLAY.SPKG);
            }
            ...
            for (int i = 0; i < SETG.Count; i++)
            {
                if (SETG[i].SGF_ == (cbxAlignments.Items.Cast<SGFF>().ToList()).Find(item => item.name == "Norm").SGF_)
                {
                    ...
                    setg.rec = SETG[i].rec;
                    setg.SETP = SETG[i].SETP;
                    setg.SGF_ = SETG[i].SGF_;
                    setg.SF__ = SETG[i].SF__;
                    setg.x___ = SETG[i].x___;
                    setg.y___ = SETG[i].y___;
                    setg.fx__ = SETG[i].fx__;
                    setg.fy__ = SETG[i].fy__;
                    setg.anm_ = SETG[i].anm_;
                    setg.dir_ = SETG[i].dir_;
                    setg.fanm = SETG[i].fanm;
                    setg.fdir = SETG[i].fdir;
                    setg.active = true;
            ...
            {
                List<SETG> alignmentSETG = new List<SETG>();
                foreach (SETG setg in SETG)
                {
                    if (setg.SGF_ == ((SGFF)cbxAlignments.SelectedItem).SGF_)
                    {
                        SETG newSETG = new SETG();

                        newSETG.rec = setg.rec;
                        newSETG.SETP = setg.SETP;
                        newSETG.SGF_ = setg.SGF_;
                        newSETG.SF__ = setg.SF__;
                        newSETG.x___ = setg.x___;
                        newSETG.y___ = setg.y___;
                        newSETG.fx__ = setg.fx__;
                        newSETG.fy__ = setg.fy__;
                        newSETG.anm_ = setg.anm_;
                        newSETG.dir_ = setg.dir_;
                        newSETG.fanm = setg.fanm;
                        newSETG.fdir = setg.fdir;
                        newSETG.active = true;

                        alignmentSETG.Add(newSETG);
                    }
                }
                alignmentSETG = alignmentSETG.OrderBy(s => s.SETP).Cast<SETG>().ToList();
                ...
                for (int i = 0; i < alignmentSETG.Count; i++)
                {
                    ...
                    newSETG.rec = alignmentSETG[i].rec;
                    newSETG.SETP = alignmentSETG[i].SETP;
                    newSETG.SGF_ = alignmentSETG[i].SGF_;
                    newSETG.SF__ = alignmentSETG[i].SF__;
                    newSETG.x___ = alignmentSETG[i].x___;
                    newSETG.y___ = alignmentSETG[i].y___;
                    newSETG.fx__ = alignmentSETG[i].fx__;
                    newSETG.fy__ = alignmentSETG[i].fy__;
                    newSETG.anm_ = alignmentSETG[i].anm_;
                    newSETG.dir_ = alignmentSETG[i].dir_;
                    newSETG.fanm = alignmentSETG[i].fanm;
                    newSETG.fdir = alignmentSETG[i].fdir;
                    newSETG.active = alignmentSETG[i].active;

                    _PLAY.SETG.Insert(alignSETGindex, newSETG);
                }
            }
            ...
            for (int i = 0; i < _PLAY.posos.Count(); i++)
            {
                ...
                _PLAY.posos[i].EPos = _PLAY.SETP[i].EPos;
                _PLAY.posos[i].DPos = _PLAY.SETP[i].DPos;
            }
            ...
            #region Get PBAI for current play

            if (PBAI != null)
            {
                _PLAY.PBAI = new List<PBAI>();

                for (int i = 0; i < PBAI.Count; i++)
                {
                    if (PBAI[i].PLYL == ((PBPL)cbxPlays.SelectedItem).PLYL)
                    {
                        _PLAY.PBAI.Add(new PBAI
                        {
                            rec = PBAI[i].rec,
                            BOKL = PBAI[i].BOKL,
                            PLYL = PBAI[i].PLYL,
            ...
            #region Get PBAU for current play

            if (PBAU != null)
            {
                _PLAY.PBAU = new List<PBAU>();

                //for (int i = 0; i < PBAU.Count; i++)
                //{
                //    if (_PLAY.PBPL[0].PLYL == ((PBPL)cbxPlays.SelectedItem).PLYL)
                //    {
                //        _PLAY.PBAU.Add(new PBAU
                //        {
                //            rec = PBAU[i].rec,
                //            BOKL = PBAU[i].BOKL,
                //            PBPL = PBAU[i].PBPL,
            ...
            #region Get PLCM for current play

            if (PLCM != null)
            {
                _PLAY.PLCM = new List<PLCM>();

                for (int i = 0; i < PLCM.Count; i++)
                {
                    if (PLCM[i].PLYL == ((PBPL)cbxPlays.SelectedItem).PLYL)
                    {
                        _PLAY.PLCM.Add(new PLCM
                        {
                            rec = PLCM[i].rec,
                            per1 = PLCM[i].per1,
                            dir1 = PLCM[i].dir1,
            ...
            #region Get PPCT for current play

            if (PPCT != null)
            {
                _PLAY.PPCT = new List<PPCT>();

                for (int i = 0; i < PPCT.Count; i++)
                {
                    if (PPCT[i].PLYL == ((PBPL)cbxPlays.SelectedItem).PLYL)
                    {
                        _PLAY.PPCT.Add(new PPCT
                        {
                            rec = PPCT[i].rec,
                            PLYL = PPCT[i].PLYL,
                            conp = PPCT[i].PLYL,
            ...
            #region Get SDEF for current play

            if (SDEF != null)
            {
                _PLAY.SDEF = new List<SDEF>();

                for (int i = 0; i < SDEF.Count; i++)
                {
                    if (SDEF[i].PLYL == ((PBPL)cbxPlays.SelectedItem).PLYL)
                    {
        ...
        private List<SPKG> GetSPKG()
        {
            ...
            // Get Tableprops based on the selected index
            if (!TDB.TDBTableGetProperties(OpenIndex, TableNames.Find(item => item.name == "SPKG").rec, ref TableProps))
                return null;
            ...
            for (int i = 0; i < TableProps.RecordCount; i++)
            {
                ...
                {
                    rec = i,
                    SPF_ = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("SPKG"), StrReverse("SPF_"), i),
                    poso = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("SPKG"), StrReverse("poso"), i),
                    DPos = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("SPKG"), StrReverse("DPos"), i),
                    EPos = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("SPKG"), StrReverse("EPos"), i)
                });
            }
            return _SPKG;
        }

        private List<SPKF> GetSPKF()
        {
            ...
            // Get Tableprops based on the selected index
            if (!TDB.TDBTableGetProperties(OpenIndex, TableNames.Find(item => item.name == "SPKF").rec, ref TableProps))
                return null;
            ...
            for (int i = 0; i < TableProps.RecordCount; i++)
            {
                ...
                TDB.TDBFieldGetProperties(OpenIndex, StrReverse("SPKF"), 2, ref FieldProps);

                string _name = new string((char)0, (FieldProps.Size / 8) + 1);

                TDB.TDBFieldGetValueAsString(OpenIndex, StrReverse("SPKF"), StrReverse("name"), i, ref _name);
                _name = _name.Replace(",", "");

                _SPKF.Add(new SPKF
                {
                    rec = i,
                    SETL = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("SPKF"), StrReverse("SETL"), i),
                    SPF_ = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("SPKF"), StrReverse("SPF_"), i),
                    name = _name
                });
            }
            return _SPKF;
        ...
        private List<SRFT> GetSRFT()
        {
            ...
            // Get Tableprops based on the selected index
            if (TableNames.Find(item => item.name == "SRFT") == null)
            {
                return null;
            }
            else
        ...
        private List<SDEF> GetSDEF()
        {
            ...
            // Get Tableprops based on the selected index
            if (TableNames.Find(item => item.name == "SDEF") == null)
            {
                return null;
            }
            else
            {
                TDB.TDBTableGetProperties(OpenIndex, TableNames.Find(item => item.name == "SDEF").rec, ref TableProps);
            }
            ...
            for (int i = 0; i < TableProps.RecordCount; i++)
            {
                ...
                {
                    rec = i,
                    ATCA = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("SDEF"), StrReverse("ATCA"), i),
                    PLYL = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("SDEF"), StrReverse("PLYL"), i),
                    DFLP = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("SDEF"), StrReverse("DFLP"), i),
                    STRP = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("SDEF"), StrReverse("STRP"), i),
                    STRR = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("SDEF"), StrReverse("STRR"), i)
                });
            }
            return _SDEF;
        }
        ...
        private List<PBAI> GetPBAI()
        {
            ...
            // Get Tableprops based on the selected index
            if (!TDB.TDBTableGetProperties(OpenIndex, TableNames.Find(item => item.name == "PBAI").rec, ref TableProps))
                return null;
            ...
            for (int i = 0; i < TableProps.RecordCount; i++)
            {
                ...
                {
                    rec = i,
                    BOKL = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBAI"), StrReverse("BOKL"), i),
                    PLYL = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBAI"), StrReverse("PLYL"), i),
                    AIGR = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBAI"), StrReverse("AIGR"), i),
                    prct = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBAI"), StrReverse("prct"), i)
                });
            }
            return _PBAI;
        }

        private List<PBAU> GetPBAU()
        {
            ...
            // Get Tableprops based on the selected index
            if (!TDB.TDBTableGetProperties(OpenIndex, TableNames.Find(item => item.name == "PBAU").rec, ref TableProps))
                return null;
            ...
            for (int i = 0; i < TableProps.RecordCount; i++)
            {
                ...
                {
                    rec = i,
                    BOKL = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBAU"), StrReverse("BOKL"), i),
                    PBPL = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBAU"), StrReverse("PLYL"), i),
                    FTYP = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBAU"), StrReverse("AIGR"), i),
                    pbau = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBAU"), StrReverse("AIGR"), i),
                    Flag = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PBAU"), StrReverse("prct"), i)
                });
            }
            return _PBAU;
        }

        private List<PLCM> GetPLCM()
        {
            ...
            // Get Tableprops based on the selected index
            if (!TDB.TDBTableGetProperties(OpenIndex, TableNames.Find(item => item.name == "PLCM").rec, ref TableProps))
                return null;
            ...
            for (int i = 0; i < TableProps.RecordCount; i++)
            {
                ...
                {
                    rec = i,
                    per1 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("per1"), i),
                    dir1 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("dir1"), i),
                    ply1 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("ply1"), i),
                    per2 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("per2"), i),
                    dir2 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("dir2"), i),
                    ply2 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("ply2"), i),
                    per3 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("per3"), i),
                    dir3 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("dir3"), i),
                    ply3 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("ply3"), i),
                    per4 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("per4"), i),
                    dir4 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("dir4"), i),
                    ply4 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("ply4"), i),
                    per5 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("per5"), i),
                    dir5 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("dir5"), i),
                    ply5 = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("ply5"), i),
                    PLYL = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PLCM"), StrReverse("PLYL"), i)
                });
            }
            return _PLCM;
        }

        private List<PPCT> GetPPCT()
        {
            ...
            // Get Tableprops based on the selected index
            if (TableNames.Find(item => item.name == "PPCT") == null)
            {
                return null;
            }
            else
            {
                TDB.TDBTableGetProperties(OpenIndex, TableNames.Find(item => item.name == "PPCT").rec, ref TableProps);
            }
            ...
            for (int i = 0; i < TableProps.RecordCount; i++)
            {
                ...
                {
                    rec = i,
                    PLYL = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PPCT"), StrReverse("PLYL"), i),
                    conp = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PPCT"), StrReverse("conp"), i),
                    recr = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(OpenIndex, StrReverse("PPCT"), StrReverse("recr"), i)
                });
            }
            return _PPCT;
        }
        ...
        private void cmsEditPSAL_Copy_Click(object sender, EventArgs e)
        {
            ...
            foreach (PLYS poso in PLYS_Copy) Console.WriteLine(poso);

            Clipboard.Clear();
            Clipboard.SetDataObject(PLYS_Copy, true);
        }

        private void cmsEditPSAL_Paste_Click(object sender, EventArgs e)
        {
            List<PLYS> newPLYS = (List<PLYS>)Clipboard.GetDataObject().GetData(typeof(List<PLYS>));

            if (newPLYS != null)
            {
                PlayCopy.PLYS = newPLYS;
            }

            foreach (PLYS ply in PlayCopy.PLYS)
            {
                PSAL pslo = PSLO.FirstOrDefault(psal => psal.psal == ply.PSAL);
                PSAL psld = PSLD.FirstOrDefault(psal => psal.psal == ply.PSAL);

                if (pslo != null || psld != null)
                {
        ...
        {
            if ((List<PLYS>)Clipboard.GetDataObject().GetData(typeof(List<PLYS>)) != null)
            {
                cmsEditPSAL_Paste.Enabled = true;
            }
        }

        private void Copy_Play(object sender, EventArgs e)
        {
            CustomPlaybookPLAY CPBplay = new CustomPlaybookPLAY(Play);

            Console.WriteLine(CPBplay);

            Clipboard.Clear();
            Clipboard.SetDataObject(CPBplay, true);
        }

        private void Copy_SubFormation(object sender, EventArgs e)
        {
            Clipboard.Clear();
            CustomPlaybookSubFormation CPBsubFormation = new CustomPlaybookSubFormation();
            CPBsubFormation = new CustomPlaybookSubFormation();

            CPBsubFormation.Plays = new List<CustomPlaybookPLAY>();
            for (int i = 0; i < cbxPlays.Items.Count; i++)
            {
                cbxPlays.SelectedIndex = i;
                CPBsubFormation.Plays.Add(new CustomPlaybookPLAY(Play));
            }

            CPBsubFormation.SETL = SETL.Where(set => set.setl == ((SETL)cbxSubFormations.SelectedItem).setl).Cast<SETL>().ToList();
            CPBsubFormation.PGPL = (PGPL.Where(set => set.SETL == ((SETL)cbxSubFormations.SelectedItem).setl)).Where(play => play.PLYL == ((PBPL)cbxPlays.SelectedItem).PLYL).Cast<PGPL>().ToList();
            CPBsubFormation.SPKF = SPKF.Where(package => package.SETL == ((SETL)cbxSubFormations.SelectedItem).setl).Cast<SPKF>().ToList();
            CPBsubFormation.SPKG = new List<SPKG>();
            foreach (SPKF sub in CPBsubFormation.SPKF)
            {
                CPBsubFormation.SPKG.AddRange(SPKG.Where(subs => subs.SPF_ == sub.SPF_).Cast<SPKG>().ToList());
            }
            CPBsubFormation.SETP = SETP.Where(poso => poso.SETL == ((SETL)cbxSubFormations.SelectedItem).setl).Cast<SETP>().ToList();
            CPBsubFormation.SGFF = SGFF.Where(poso => poso.SETL == ((SETL)cbxSubFormations.SelectedItem).setl).Cast<SGFF>().ToList();
            CPBsubFormation.SETG = new List<SETG>();
            foreach (SGFF motion in CPBsubFormation.SGFF)
            {
                CPBsubFormation.SETG.AddRange(SETG.Where(poso => poso.SGF_ == motion.SGF_).Cast<SETG>().ToList());
            }

            Console.WriteLine(CPBsubFormation);

            Clipboard.SetDataObject(CPBsubFormation, true);
        }

        private void Copy_Formation(object sender, EventArgs e)
        {
            Clipboard.Clear();
            CustomPlaybookFormation CPBformation = new CustomPlaybookFormation();
            CPBformation.SubFormations = new List<CustomPlaybookSubFormation>();
            for (int i = 0; i < cbxSubFormations.Items.Count; i++)
            {
                CustomPlaybookSubFormation CPBsubFormation = new CustomPlaybookSubFormation();
                CPBsubFormation.Plays = new List<CustomPlaybookPLAY>();
                cbxSubFormations.SelectedIndex = i;
                for (int n = 0; n < cbxPlays.Items.Count; n++)
                {
                    cbxPlays.SelectedIndex = n;
                    CPBsubFormation.Plays.Add(new CustomPlaybookPLAY(Play));
                }
                CPBsubFormation.SETL = SETL.Where(set => set.setl == ((SETL)cbxSubFormations.SelectedItem).setl).Cast<SETL>().ToList();
                CPBsubFormation.PGPL = (PGPL.Where(set => set.SETL == ((SETL)cbxSubFormations.SelectedItem).setl)).Where(play => play.PLYL == ((PBPL)cbxPlays.SelectedItem).PLYL).Cast<PGPL>().ToList();
                ...
                CPBformation.SubFormations.Add(CPBsubFormation);
            }
            CPBformation.PBFM = PBFM.Where(formation => ((CPFM)cbxFormations.SelectedItem).name.ToLower().Contains(formation.name.ToLower())).FirstOrDefault();
            CPBformation.CPFM = ((CPFM)cbxFormations.SelectedItem);

            Console.WriteLine(CPBformation);

            Clipboard.SetDataObject(CPBformation, true);
        }

        private void GetCode58Playart(object sender, EventArgs e)
        {
            if (PSLO != null)
            {
                string Code58s = "PSAL, Val1, Val2, Val3, Poso, Play, PLYL, Playbook\n";
                List<PSAL> Code58psals = PSLO.Where(psal => psal.code == 58).OrderBy(psal => psal.psal).Distinct().Cast<PSAL>().ToList();
                foreach (PSAL PSAL in Code58psals)
                {
                    try
                    {
                        PLYS plys = PLYS.Where(play => play.PSAL == PSAL.psal).FirstOrDefault();
                        PBPL pbpl = PBPL.Where(play => play.PLYL == plys.PLYL).FirstOrDefault();
                        STID stid = STID.Where(set => set.SETL == pbpl.SETL).FirstOrDefault();
                        PBFI pbfi = PBFI.Where(id => id.BOKL == stid.BOKL).FirstOrDefault();
                        Code58s += +PSAL.psal + ", " + PSAL.val1 + ", " + PSAL.val2 + ", " + PSAL.val3 + ", " + plys.poso + ", " + pbpl.name + ", " + pbpl.PLYL + ", " + pbfi.name + "\n";
                    }
                    catch
                    {
                    }
                }
                Console.WriteLine(Code58s);
                Clipboard.Clear();
                Clipboard.SetDataObject(Code58s, true);
                MessageBox.Show("Code 58 PSAL data copied to clipboard as csv");
            }
        }

        private void searchPlayByPLYL(object sender, EventArgs e)
        {
            findPlay();
        }

        private void txtPLYLSearch_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter && PBPL != null)
            {
                findPlay();
            }
        }
        ...
        public void findPlay()
        {
            if (PBPL == null) return;
            int PLYL = 0;
            bool number = Int32.TryParse(txtPLYLSearch.Text, out PLYL);
            if (number)
            {
                try
                {
                    PBPL pbpl = PBPL.Where(play => play.PLYL == PLYL).FirstOrDefault();
                    SETL setl = SETL.Where(subFormation => subFormation.setl == pbpl.SETL).FirstOrDefault();
                    CPFM cpfm = CPFM.Where(formation => formation.FORM == setl.FORM).FirstOrDefault();
                    cbxFormations.SelectedItem = cpfm;
                    cbxSubFormations.SelectedItem = setl;
                    cbxPlays.SelectedItem = pbpl;
                }
                catch
                {
                    MessageBox.Show("PLYL not found");
                }
            }
            else
            {
                try
                {
                    PBPL pbpl = PBPL.Where(play => play.name == txtPLYLSearch.Text).FirstOrDefault();
                    SETL setl = SETL.Where(subFormation => subFormation.setl == pbpl.SETL).FirstOrDefault();
                    CPFM cpfm = CPFM.Where(formation => formation.FORM == setl.FORM).FirstOrDefault();
                    cbxFormations.SelectedItem = cpfm;
                    cbxSubFormations.SelectedItem = setl;
        ...
        private void cbxPSALColor_SelectedIndexChanged(object sender, EventArgs e)
        {
            Color PSALcolor = Play.posos[dgvPLYS.CurrentRow.Index].RouteColor.Color;
            if (((CPFM)cbxFormations.SelectedItem).FTYP == 1 || ((CPFM)cbxFormations.SelectedItem).FTYP == 2 || ((CPFM)cbxFormations.SelectedItem).FTYP == 3)
            {
                ...
                for (int i = 0; i <= 240; i++)
                {
                    Color HLScolor = ColorTranslator.FromWin32(ColorHLSToRGB(i, 120, 240));
                    //AllColors.Add(HLScolor);
                    int brightness = PSALColor.Brightness(HLScolor);
                    switch (cbxPSALColor.SelectedIndex)
                    {
                        case 0:
                            if (brightness > 208)
                            {
                            ...
                            break;
                        case 1:
                            if (brightness > 173 && brightness <= 208)
                            {
                                Colors.Add(HLScolor);
                            }
                            break;
                        case 2:
                            if (brightness > 137 && brightness <= 173)
                            {
                                Colors.Add(HLScolor);
                            }
                            break;
                        case 3:
                            if (brightness > 102 && brightness <= 137)
                            {
                                Colors.Add(HLScolor);
                            }
                            break;
                        case 4:
                            if (brightness > 66 && brightness <= 102)
                            {
                                Colors.Add(HLScolor);
                            }
                            break;
                ...
                //AllColors = AllColors.OrderBy(color => PSALColor.Brightness(color)).ThenBy(color => (int)color.GetHue()).Cast<Color>().ToList();

                //Bitmap HSL = new Bitmap(241, 1);
                //for (int i = 0; i < 241; i++)
                //{
                //    HSL.SetPixel(i, 0, AllColors[i]);
                //}
                ...
                //if (saveFileDialog1.ShowDialog() == DialogResult.OK)
                //{
                //    HSL.Save(saveFileDialog1.FileName, ImageFormat.Png);
                //}

                Colors = Colors.OrderByDescending(color => PSALColor.Brightness(color)).ThenBy(color => (int)color.GetHue()).Cast<Color>().ToList();
                ...
                cdlPSLO.CustomColors = new int[] {
                                        ...
                                        ColorTranslator.ToOle(PSALColor.QBHandoff.Color),
                                        ColorTranslator.ToOle(PSALColor.Kickoff.Color),
                                        ColorTranslator.ToOle(Colors[(int)(1 * (Colors.Count / 8)) - 1]),
                                        ColorTranslator.ToOle(Colors[(int)(2 * (Colors.Count / 8)) - 1]),
                                        ColorTranslator.ToOle(Colors[(int)(3 * (Colors.Count / 8)) - 1]),
                                        ColorTranslator.ToOle(Colors[(int)(4 * (Colors.Count / 8)) - 1]),
                                        ColorTranslator.ToOle(Colors[(int)(5 * (Colors.Count / 8)) - 1]),
                                        ColorTranslator.ToOle(Colors[(int)(6 * (Colors.Count / 8)) - 1]),
                                        ColorTranslator.ToOle(Colors[(int)(7 * (Colors.Count / 8)) - 1]),
                                        ColorTranslator.ToOle(Colors[Colors.Count - 1])
                                      };

                cdlPSLO.Color = ColorTranslator.FromWin32(cdlPSLO.CustomColors[8]);
                cdlPSLO.ShowDialog();
               }
