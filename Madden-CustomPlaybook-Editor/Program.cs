...
namespace MaddenCustomPlaybookEditor
{
    ...
    public class CustomPlaybookPLAY
    {
        ...
        public CustomPlaybookPLAY(PLAY Play)
        {
            List<List<PSAL>> psal = new List<List<PSAL>>();
            List<ARTL> artl = new List<ARTL>();
            for (int i = 0; i < Play.posos.Count(); i++)
            {
                List<PSAL> steps = new List<PSAL>();
                foreach (PSALroute.Steps step in Play.posos[i].steps)
                {
                    steps.Add(new PSAL
                    {
                        rec = step.rec,
